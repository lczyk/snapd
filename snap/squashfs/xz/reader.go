...
// Close releases resources held by the Reader, in particular the LZMA2
// dictionary buffer (8MB by default) which is returned to a per-capacity
// sync.Pool for reuse by the next NewReader. Close is optional; not
// calling it just means the dict gets GC'd as normal.
func (r *Reader) Close() error {
	if r.sr != nil && r.sr.br != nil && r.sr.br.closer != nil {
		err := r.sr.br.closer.Close()
		r.sr.br.closer = nil
		return err
	}
	return nil
