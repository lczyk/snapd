...
// will be used as reader buffer. Pulls from a sync.Pool keyed by
// capacity to avoid the dominant per-decode allocation.
func newDecoderDict(dictCap int) (d *decoderDict, err error) {
	if !(1 <= dictCap && int64(dictCap) <= MaxDictCap) {
		return nil, errors.New("lzma: dictCap out of range")
	}
	return acquireDecoderDict(dictCap)
}

// newDecoderDictUnpooled allocates a fresh decoderDict, bypassing the
// pool. Used by the pool itself.
func newDecoderDictUnpooled(dictCap int) (*decoderDict, error) {
	if !(1 <= dictCap && int64(dictCap) <= MaxDictCap) {
		return nil, errors.New("lzma: dictCap out of range")
	}
	return &decoderDict{buf: *newBuffer(dictCap)}, nil
}
